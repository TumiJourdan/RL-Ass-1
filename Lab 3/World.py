...
class World:
    ...
    def takeAction(self, move):
        #move is tuple or constants (up down left right respectively)
    ...
    def returnAvailableMove(self,position:tuple):
        # check which moves are inbounds
        return_moves = []
        return_coord = []
        for x in movelist:
            projectedCoordinates = position + np.array(x)
            
            if not(projectedCoordinates[0] < 0 or projectedCoordinates[1] < 0 or projectedCoordinates[0] > 3 or projectedCoordinates[1] > 3):
               return_moves.append(x)    
               return_coord.append(projectedCoordinates)  
                      
        return return_moves,return_coord
