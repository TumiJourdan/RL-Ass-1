...
def create_edge_info_list(obs):
    ...
    return edge_info
nodes = create_node_gen_load_list(obs)
edges = create_edge_info_list(obs)
print(nodes)
print(edges)
...
x = torch.zeros((num_nodes, 1), dtype=torch.float)  # Dummy feature for each node

# Step 3: Create a Data object
data = Data(x=nodes, edge_index=edge_index,edge_attr=edges,num_nodes = obs.n_sub)
